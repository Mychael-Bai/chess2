...
class ChineseChess:
    ...
    def evaluate_position_simple(self, color='black'):
                    ...
                    if game_phase == "opening":
                        ...
                        if piece[1] in ['馬', '車', '炮']:
                            if piece[0] == eval_prefix:
                                position_bonus += 30 if abs(col - 4) <= 2 else 0
                        # Penalty for early king movement
                        elif piece[1] in ['將', '帥']:
                            position_bonus -= 50
                        ...
                        if (piece[0] == 'R' and row < 5) or (piece[0] == 'B' and row > 4):
                            position_bonus += 30
                    
                    else:  # Endgame
                        # Stronger piece coordination
                        friendly_pieces = self.count_nearby_friendly_pieces(row, col, piece[0])
                        position_bonus += friendly_pieces * 20
                        # Pawn advancement bonus
                        if piece[1] in ['兵', '卒']:
                            position_bonus += self.evaluate_pawn_advancement(row, col, piece[0])
        ...
        # Weight the components based on game phase
        if game_phase == "opening":
    ...
    def _move_sorting_score(self, move):
        ...
        # Development bonus
        if self.get_game_phase() == "opening":
            if from_piece[1] in ['馬', '車', '炮']:
                if abs(from_col - 4) > abs(to_col - 4):  # Moving toward center
                    score += 30
