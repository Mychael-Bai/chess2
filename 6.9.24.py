...
class ChineseChess:
    ...
    def minimax(self, depth, alpha, beta, maximizing_player):
        """Minimax algorithm with alpha-beta pruning and positional evaluation"""
        ...
        if maximizing_player:
            ...
            moves.sort(key=self._move_sorting_score, reverse=True)  # Sort moves to improve pruning
            ...
            for from_pos, to_pos in moves:
                ...
                if not self.is_in_check('black'):
                    # Reduce search depth for non-forcing moves
                    next_depth = depth - 1
                    if captured_piece or self.is_in_check('red'):
                        next_depth += 1  # Search captures and checks deeper
                    
                    eval = self.minimax(next_depth, alpha, beta, False)
            ...
            moves.sort(key=lambda m: self._move_sorting_score(m))  # Sort moves to improve pruning
            ...
            for from_pos, to_pos in moves:
                ...
                if not self.is_in_check('red'):
                    # Reduce search depth for non-forcing moves
                    next_depth = depth - 1
                    if captured_piece or self.is_in_check('black'):
                        next_depth += 1  # Search captures and checks deeper
                    
                    eval = self.minimax(next_depth, alpha, beta, True)
    ...
    def evaluate_position_simple(self, color='black'):
        ...
        # Material and position evaluation with phase-dependent weights
        material_score = 0
        position_score = 0
        mobility_score = 0
        structure_score = 0
        ...
        for row in range(10):
            for col in range(9):
                ...
                if piece:
                    ...
                    mobility = self.calculate_piece_mobility(row, col, piece)
                    
                    # Apply scores based on piece ownership
                    if piece[0] == eval_prefix:
                        material_score += base_value
                        position_score += position_bonus
                        mobility_score += mobility
                        structure_score += self.evaluate_piece_structure(row, col, piece)
                    else:
                        material_score -= base_value
                        position_score -= position_bonus
                        mobility_score -= mobility
        ...
        # Add checkmate potential and king safety evaluations
        checkmate_potential = (self.evaluate_checkmate_potential(color) - 
        ...
        # Add the final components to the score
        score += checkmate_potential * 2
        score += king_safety
        
        return score  # Add this return statement
    ...
    def evaluate_checkmate_potential(self, color):
        ...
        # Count attacking pieces and their influence
        attackers = []
        attack_value = 0
        control_score = 0
        for dr in range(-3, 4):  # Extended range for long-range pieces
            for dc in range(-3, 4):
                r, c = king_row + dr, king_col + dc
                if 0 <= r < 10 and 0 <= c < 9:
                    piece = self.board[r][c]
                    if piece and piece[0] == color[0].upper():
                        # Record attacker's position and type
                        attackers.append((r, c, piece[1]))
                        # Higher value for powerful pieces near the king
                        distance = max(abs(dr), abs(dc))
                        if piece[1] == '車':
                            attack_value += 150 // distance
                            control_score += 40
                        elif piece[1] == '馬':
                            attack_value += 120 // distance
                            control_score += 30
                        elif piece[1] == '炮':
                            attack_value += 100 // distance
                            control_score += 35
                        else:
                            attack_value += 50 // distance
                            control_score += 20
        
        score += len(attackers) * 50 + attack_value + control_score
        
        # Analyze king's escape routes
        escape_squares = 0
        safe_squares = 0
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                ...
                if 0 <= r < 10 and 0 <= c < 9:
                    if self.is_valid_move((king_row, king_col), (r, c)):
                        escape_squares += 1
                        # Test if the escape square is safe
        ...
        # Higher scores for fewer escape options
        score += (9 - escape_squares) * 80
        score += (9 - safe_squares) * 100
        
        # Additional bonuses for specific attack patterns
        if len(attackers) >= 2:
            score += 200  # Bonus for multiple attackers
            
            # Check for crossfire attacks
            has_horizontal = False
            has_vertical = False
            for attacker_row, attacker_col, piece_type in attackers:  # Fixed: use proper variables from attackers
                if piece_type in ['車', '炮']:
                    if abs(king_row - attacker_row) == 0:  # Fixed: use attacker_row
                        has_horizontal = True
                    if abs(king_col - attacker_col) == 0:  # Fixed: use attacker_col
                        has_vertical = True
            if has_horizontal and has_vertical:
                score += 300  # Bonus for cross attacks
        
        # Extra bonus for current check
        if self.is_in_check(opposing_color):
            score += 500
            if safe_squares == 0:  # Check with no safe squares
                score += 1000
        
        return score
        
    def _move_sorting_score(self, move):
        """Score moves for sorting with enhanced tactical evaluation"""
        ...
        # Check-related scores
        if self.is_checkmate('red'):
            score += 20000  # Highest priority for checkmate
        elif self.is_in_check('red'):
            score += 1000
            # Additional score for checks with multiple threats
            check_count = 0
            for r in range(10):
                for c in range(9):
                    piece = self.board[r][c]
                    if piece and piece[0] == 'B':
                        kings = self.find_kings()
                        if kings[0] and self.is_valid_move((r, c), kings[0]):
                            check_count += 1
            score += check_count * 200
        ...
        if from_piece[0] == 'B':  # Black pieces
            if to_row > 4:  # Crossed river
                score += 40
                if from_piece[1] in ['車', '馬', '炮']:
                    score += 30
        else:  # Red pieces
            if to_row < 5:  # Crossed river
                score += 40
                if from_piece[1] in ['車', '馬', '炮']:
                    score += 30
        ...
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                ...
                if 0 <= r < 10 and 0 <= c < 9:
                    piece = self.board[r][c]
                    if piece and piece[0] == from_piece[0]:
                        friendly_pieces_nearby += 1
        score += friendly_pieces_nearby * 10
    ...
    def count_nearby_friendly_pieces(self, row, col, color):
        """Count friendly pieces in the 8 adjacent squares"""
        count = 0
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                if dr == 0 and dc == 0:
    ...
    def evaluate_pawn_advancement(self, row, col, color):
        """Score based on how far the pawn has advanced"""
        if color == 'R':
            if not self.flipped:
                return (9 - row) * 10
            else:
    ...
    def calculate_piece_mobility(self, row, col, piece):
        """Calculate how many moves a piece can make"""
        mobility = 0
        for to_row in range(10):
            for to_col in range(9):
                if self.is_valid_move((row, col), (to_row, to_col)):
                    ...
                    if not self.is_in_check('red' if piece[0] == 'R' else 'black'):
                        mobility += 1
    ...
    def evaluate_piece_structure(self, row, col, piece):
        """Evaluate the structural value of a piece's position"""
        score = 0
        color = piece[0]
        piece_type = piece[1]
        
        # Central control bonus for major pieces
        if piece_type in ['車', '馬', '炮']:
            if 2 <= col <= 6:
                score += 20
            # Extra bonus for controlling central files in opponent's half
            if (color == 'R' and row < 5) or (color == 'B' and row > 4):
                score += 30
        ...
        if piece_type in ['兵', '卒']:
            ...
            for dc in [-1, 1]:
                new_col = col + dc
                if 0 <= new_col < 9:
                    neighbor = self.board[row][new_col]
                    if neighbor and neighbor[1] in ['兵', '卒'] and neighbor[0] == color:
                        score += 15
        ...
        # Piece coordination
        nearby_friends = self.count_nearby_friendly_pieces(row, col, color)
