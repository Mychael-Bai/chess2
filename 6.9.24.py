...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, to_pos, piece):
        """
        Determine 前/后 based on the visual perspective of the board.
        When flipped, the perspective changes for both sides.
        """
        ...
        for row in range(10):
            ...
            if current_piece and current_piece[0] == piece_color and current_piece[1] == piece_type and row != to_row:
                identical_positions.append(row)
        identical_positions.append(from_row)
        
        if len(identical_positions) == 2:
            # Sort positions based on the visual perspective
            if (piece_color == 'R' and not self.flipped) or (piece_color == 'B' and self.flipped):
                # Pieces at the bottom of the board - higher row numbers are "后"
                identical_positions.sort(reverse=True)  # Sort in descending order
                if from_row == identical_positions[0]:
                    return "前"
                else:
                    return "后"
            else:
                # Pieces at the top of the board - lower row numbers are "后"
                identical_positions.sort()  # Sort in ascending order
                if from_row == identical_positions[0]:
                    return "前"
                else:
                    return "后"
    ...
    def get_move_text(self, from_pos, to_pos, piece):
        ...
        # Determine piece's perspective (whether it's playing from top or bottom)
        playing_from_bottom = (piece_color == 'R' and not self.flipped) or (piece_color == 'B' and self.flipped)
        
        # Get column numbers based on piece color and board orientation
        if piece_color == 'R':
            columns = ['一', '二', '三', '四', '五', '六', '七', '八', '九']
            if playing_from_bottom:
