...
class ChineseChess:
    ...
    def __init__(self):
        ...
        # Set initial window size to base minimum
        self.window.geometry(f"{self.base_min_width}x{self.min_height}")
    ...
    def on_window_configure(self, event):
        ...
        if not hasattr(self, 'processing_resize'):
            self.processing_resize = False
        ...
        if event.widget == self.window and not self.processing_resize:
            try:
                ...
                if current_width < min_width or current_height < self.min_height:
    ...
    def toggle_records(self):
        ...
        if self.records_frame.winfo_ismapped():
            ...
            # Set minimum width back to base
            self.window.minsize(self.base_min_width, self.min_height)
            
            # If window is wider than necessary, shrink it
            if current_width > self.base_min_width:
                new_width = self.base_min_width
                self.window.geometry(f"{new_width}x{current_height}")
            
        else:
            # Show records
            self.records_frame.pack(side=tk.LEFT, before=self.board_frame, padx=10)
            
            # Set minimum width to accommodate records
            self.window.minsize(self.records_min_width, self.min_height)
            
            # If window is too narrow for records, expand it
            if current_width < self.records_min_width:
                self.window.geometry(f"{self.records_min_width}x{current_height}")
