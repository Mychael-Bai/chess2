...
class ChineseChess:
    ...
    def toggle_records(self):
        ...
        required_width = self.base_min_width  # Start with base width
        
        if self.records_seen:
            required_width = self.records_min_width  # Add width for records
        
        if self.piece_setting_mode and self.pieces_frame:
            required_width += 200  # Add width for pieces frame
        
        # Set new minimum size
        self.window.minsize(required_width, self.min_height)
        
        # If current window is smaller than required, resize it
        if current_width < required_width:
            new_geometry = f"{required_width}x{current_height}"
            self.window.geometry(new_geometry)
        
        # Always ensure pieces frame remains visible if in piece setting mode
        if self.piece_setting_mode and self.pieces_frame:
            self.pieces_frame.pack_forget()
            self.pieces_frame.pack(side=tk.RIGHT, padx=15)
