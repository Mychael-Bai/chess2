...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("Chinese Chess 6.9.43 (window width correct)")
    ...
    def on_window_configure(self, event):
        ...
        if event.widget == self.window and not hasattr(self, '_configuring'):
            try:
                ...
                if self.piece_setting_mode:
                    min_width = self.records_min_width + 193  # Extra space for pieces
                elif self.records_seen:
                    min_width = self.records_min_width
                else:
                    min_width = self.base_min_width
                    
                # Only enforce minimum size if absolutely necessary
                if current_width < min_width or current_height < self.min_height:
                    new_width = max(min_width, current_width)
                    new_height = max(self.min_height, current_height)
                    # Use after_idle to prevent immediate geometry changes
                    self.window.after_idle(
                        lambda: self.window.geometry(f"{new_width}x{new_height}")
                    )
