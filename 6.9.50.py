...
class ChineseChess:
    ...
    def switch_colors(self):
        ...
        # Temporarily disable window updates to prevent flashing
        ...
        # Handle piece setting mode without recreating frames
        if self.piece_setting_mode:
            self.piece_to_place = None
            self.selected_instance_id = None
            
            # Update pieces frame display order
            if self.pieces_frame:
                self.pieces_frame.pack_forget()
                self.create_pieces_frame()
                self.pieces_frame.pack(side=tk.RIGHT, padx=15)
        ...
        # Restore window dimensions and visibility
