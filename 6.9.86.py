...
class ChineseChess:
    ...
    def __init__(self):
        ...
        # Add this after creating the style object
        style = ttk.Style()
        style.configure('Custom.TButton', font=('SimSun', 12), focuscolor='none')
        # On Windows, you might also need:
        style.map('Custom.TButton',
            focuscolor=[('active', 'none')],
            background=[('active', '#d9d9d9')],  # Keep the press effect but remove focus rectangle
        )
    ...
    def start_replay(self):
        ...
        if self.restart_state == True:
            self.board = [row[:] for row in self.board_copy_restart]
            self.draw_board()
        else:
