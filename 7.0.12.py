...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.board_copy_not_flipped = [[None for _ in range(9)] for _ in range(10)]  # Initialize empty board copy
        self.board_copy_flipped = [[None for _ in range(9)] for _ in range(10)]  # Initialize empty board copy
        self.original_red_side = None
        self.rotate_board_record = False
        ...
        self.window.title("Chinese Chess 7.0.09 (restart ok enven color switched)")
    ...
    def on_click(self, event):
        ...
        if len(self.move_history) == 0:
            self.board_copy_not_flipped = [row[:] for row in self.board]
            self.original_red_side = 'B'
    ...
    def make_ai_move(self):
        ...
        if len(self.move_history) == 0:
            self.board_copy_flipped = [row[:] for row in self.board]
            self.original_red_side = 'T'
    ...
    def switch_colors(self):
        ...
        if self.rotate_board_record == False:
            
            # Rotate original pieces 180 degrees
            if self.original_red_side == 'B':
                self.rotate_board_record = True
                for row in range(10):
                    for col in range(9):
                        if self.board_copy_not_flipped[row][col]:
                            new_row = 9 - row
                            new_col = 8 - col
                            self.copy_switch_board[new_row][new_col] = self.board_copy_not_flipped[row][col]
            
            elif self.original_red_side == 'T':
                self.rotate_board_record = True
                for row in range(10):
                    for col in range(9):
                        if self.board_copy_flipped[row][col]:
                            new_row = 9 - row
                            new_col = 8 - col
                            self.copy_switch_board[new_row][new_col] = self.board_copy_flipped[row][col]
    ...
    def prev_replay_move(self):
        ...
        elif self.current_replay_index == 1:
    ...
    def restart_game(self):
        ...
        self.board = [row[:] for row in self.copy_switch_board]
