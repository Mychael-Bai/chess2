...
class ChineseChess:
    ...
    def select_piece_from_canvas(self, event, canvas, instance_id, piece):
        ...
        if items:
            ...
            # Store the selected piece info including original position and dimensions
        ...
        if self.piece_setting_mode:
            ...
            if 0 <= row < 10 and 0 <= col < 9:
                if self.piece_to_place and self.source_canvas:  # If we have a piece selected from the side panel
                    ...
                    # Store the placement info for this piece
                    self.board_piece_info = {
                        'row': row,
                        'col': col,
                        'original_info': self.original_piece_info
                    }
                    ...
                    if piece:
                        # Check if this is a piece that was placed from the side panel
                        if hasattr(self, 'board_piece_info') and \
                           self.board_piece_info['row'] == row and \
                           self.board_piece_info['col'] == col:
                            # Restore the piece to the side panel
                            original_info = self.board_piece_info['original_info']
                            canvas = original_info['canvas']
                            x = original_info['x']
                            y = original_info['y']
                            piece = original_info['piece']
                            instance_id = original_info['instance_id']
                            ...
                            for item in [circle, text]:
                                canvas.tag_bind(instance_id, '<Button-1>', 
                                    lambda e, c=canvas, i=instance_id, p=piece: 
                            ...
                            self.highlighted_positions = []
                            self.draw_board()
                            delattr(self, 'board_piece_info')
                        else:
                            # Regular piece movement on board
                            self.piece_to_place = piece
                            self.board[row][col] = None
                            self.highlighted_positions = []
                            self.draw_board()
