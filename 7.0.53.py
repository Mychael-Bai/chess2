    ...
    def on_click(self, event):
        ...
        if self.piece_setting_mode:
            ...
            if 0 <= row < 10 and 0 <= col < 9:
                if self.piece_to_place:  # If we have a piece selected from the side panel
                    # Place the new piece
                    self.board[row][col] = self.piece_to_place
                    self.highlighted_positions = [(row, col)]
                    
                    # Remove the piece from the source canvas first
                    if self.source_canvas and self.selected_instance_id:
                        # Find all items with the selected instance_id
