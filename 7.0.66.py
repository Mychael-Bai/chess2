...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.window.title("Chinese Chess 7.0.61 (latest version, the replay length of move history is ok and in piece setting mode, the piece position can be modified)")
    ...
    def is_valid_piece_position(self, piece, row, col):
        ...
        if piece_type in ['帥', '將']:  # King
            ...
            if not self.flipped:
                if is_red and row < 5:  # Red elephant cannot cross to black's side
                    return False
                if not is_red and row > 4:  # Black elephant cannot cross to red's side
                    return False
            else:
                if is_red and row > 4:  # Red elephant cannot cross to black's side
                    return False
                if not is_red and row < 5:  # Black elephant cannot cross to red's side
                    return False
        
        elif piece_type in ['兵', '卒']:  # Pawn
            # Pawns can be anywhere after crossing the river
            pass  # No position restrictions for pawns as they can be anywhere after crossing the river
    ...
    def toggle_piece_setting_mode(self):
        ...
        if self.piece_setting_mode == False:
            # Validate piece positions before exiting piece setting mode
            if not self.validate_piece_positions():
                # Show warning and revert to piece setting mode
                self.show_centered_warning("Invalid Positions", "pieces not in correct positions")
                self.piece_setting_mode = True
                
                # Update button text back to "完成摆放" since we're staying in piece setting mode
                self.set_pieces_button.config(text="完成摆放")
                return
            
            # If validation passes, proceed with existing code
            if self.move_history:
                self.game_history.append(self.move_history)
            self.move_history = []
            self.move_history_records = []
