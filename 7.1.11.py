...
class MCTSNode:
    def __init__(self, state, flip_state, parent=None, move=None, player=None):
...
class MCTS:
    def __init__(self, initial_state, flip_state, player, iterations=1000):
        ...
        self.root = MCTSNode(
            state=initial_state, 
            flip_state=flip_state,  # Pass game rules to node
            player=player
        )
...
class MainGame:
    ...
    def is_valid_elephant_move(self, from_pos, to_pos):
        ...
        return not self.board[blocking_row][blocking_col]
    ...
    def is_valid_horse_move(self, from_pos, to_pos):
        ...
        if row_diff == 2:
            ...
            return not self.board[blocking_row][from_col]
        else:
            blocking_col = from_col + (1 if to_col > from_col else -1)
            return not self.board[from_row][blocking_col]
    ...
    def is_valid_chariot_move(self, from_pos, to_pos):
        ...
        if from_row == to_row:
            ...
            return not any(self.board[from_row][col] for col in range(start_col, end_col))
        else:
            start_row = min(from_row, to_row) + 1
            end_row = max(from_row, to_row)
            return not any(self.board[row][from_col] for row in range(start_row, end_row))
    ...
    def is_valid_cannon_move(self, from_pos, to_pos):
        ...
        return pieces_between == 1 if self.board[to_row][to_col] else pieces_between == 0
