...
class MCTSNode:
    def __init__(self, state, parent=None, move=None, color='black'):
    ...
    def _is_valid_move(self, from_pos, to_pos):
        ...
        # Use the chess instance's validation methods
        piece_type = piece[1]
        if piece_type in ['帥', '將']:  # General/King
            return self.chess.is_valid_general_move(from_pos, to_pos)
        elif piece_type in ['仕', '士']:  # Advisor
            return self.chess.is_valid_advisor_move(from_pos, to_pos)
        elif piece_type in ['相', '象']:  # Elephant
            return self.chess.is_valid_elephant_move(from_pos, to_pos)
        elif piece_type == '馬':  # Horse
            return self.chess.is_valid_horse_move(from_pos, to_pos)
        elif piece_type == '車':  # Chariot
            return self.chess.is_valid_chariot_move(from_pos, to_pos)
        elif piece_type == '炮':  # Cannon
            return self.chess.is_valid_cannon_move(from_pos, to_pos)
        elif piece_type in ['兵', '卒']:  # Pawn
            return self.chess.is_valid_pawn_move(from_pos, to_pos)
        return False
...
class MCTS:
    ...
    def simulate(self, node):
        ...
        while moves_count < max_moves:
            ...
            # Check for checkmate
            if self.chess.is_checkmate(color):
