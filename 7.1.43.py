...
class MCTS:
    ...
    def get_best_move(self):
        ...
        if not self.running or not self.root.children:
            return None
        best_child = max(self.root.children, key=lambda n: n.visits)
        return best_child.move

    def stop(self):
        """Stop MCTS calculations"""
        self.running = False
...
class ChineseChess:
    ...
    def stop_game(self):
        ...
        for thread in threading.enumerate():
            if thread.name == "AI_Thread" and hasattr(thread, 'mcts'):
                thread.mcts.stop()  # Stop MCTS calculations
