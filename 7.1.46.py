...
class ChineseChess:
    ...
    def __init__(self):
        ...
        # Create terminate game button (add this first, before other buttons)
        self.terminate_button = ttk.Button(
            self.button_frame,
            text="终止对局",
            command=self.terminate_game,
            width=8,
            style='Custom.TButton'
        )
        self.terminate_button.pack(pady=5, side=tk.TOP)  # Pack it first so it appears at the top
    ...
    def terminate_game(self):
        """Terminate the current game and reset the board state"""
        # Stop any running AI thread
        self.ai_thread_running = False
        if self.ai_thread and self.ai_thread.is_alive():
            # Wait a short time for the AI thread to clean up
        ...
        # Clear move history and records
        self.move_history = []
        self.move_history_records = []
        self.move_history_numbers = []
        ...
        self.show_centered_warning("游戏终止", "对局已终止")
    ...
    def make_ai_move(self):
        ...
        def ai_thread():
            try:
                if not self.ai_thread_running:  # Check if we should exit
                    return
                ...
                if not self.ai_thread_running:  # Check again before heavy computation
                    return
                ...
                if not self.ai_thread_running:  # Check again before getting best move
                    return
                    
                best_move = mcts.get_best_move()

                if best_move and self.ai_thread_running:
        ...
        import threading
        if self.ai_thread_running:  # If there's already an AI thread running
            return  # Don't start another one
            
        self.ai_thread_running = True
        self.ai_thread = threading.Thread(target=ai_thread)
    ...
    def set_button_states_for_gameplay(self):
        ...
        self.terminate_button.config(state=tk.NORMAL)    # Keep terminate button enabled
