...
class MCTS:
    ...
    def find_mate_in_n(self, board, color, n, start_time=None):
        ...
        if time.time() - start_time > 20:  # 20 seconds limit
        ...
        checking_moves = []
        for move in moves:
            # Check time limit in the moves loop
        ...
        if n > 1 and checking_moves:
            for move in priority_moves:
                # Check time limit in the priority moves loop
                if time.time() - start_time > 20:
                    raise TimeoutError("Checkmate search timeout")
                ...
                for opp_move in opponent_moves:
                    # Check time limit in the opponent moves loop
                    if time.time() - start_time > 20:
                        raise TimeoutError("Checkmate search timeout")
                    ...
                    try:
                        ...
                        raise  # Propagate the timeout up
    ...
    def get_best_move(self):
        ...
        if not self.validator.is_in_check(self.root.color):
            ...
            checkmate_time_limit = 20  # 20 seconds limit for checkmate search
            ...
            if self.pieces_near_king(self.root.state, self.root.color, self.validator):
                for n in range(2, self.max_mate_depth + 1):
                    ...
                    if time.time() - checkmate_search_start > checkmate_time_limit:
                        return None  # Break out if exceeded time limit
                    ...
                    try:
                        ...
                        return None  # Handle timeout from find_mate_in_n
