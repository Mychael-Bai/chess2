...
class ChineseChess:
    ...
    def create_history_menu(self):
        ...
        self.history_var.set("选择对局记录")  # Default text
    ...
    def load_selected_game(self, event=None):
        ...
        try:
            ...
            # Display headers
            self.move_text.insert(tk.END, f"日期: {headers.get('Date', '未知')}\n")
            self.move_text.insert(tk.END, f"红方: {headers.get('Red', '未知')}\n")
            self.move_text.insert(tk.END, f"黑方: {headers.get('Black', '未知')}\n")
            self.move_text.insert(tk.END, f"结果: {headers.get('Result', '未知')}\n")
            self.move_text.insert(tk.END, "\n走子记录:\n")
            
            # Format and display moves
            moves = moves_text.split()
            move_num = 1
            for i in range(0, len(moves)-1, 3):  # Skip every third item (move numbers)
                if i+1 < len(moves):
                    red_move = moves[i+1]
                    black_move = moves[i+2] if i+2 < len(moves) else ""
                    self.move_text.insert(tk.END, f"{move_num}. {red_move} {black_move}\n")
                    move_num += 1
    ...
    def save_game_to_pgn(self):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            # Write PGN headers
            f.write('[Event "Chinese Chess Game"]\n')
            ...
            # Write game result
            if self.flipped:
                f.write("1-0" if self.current_player == 'black' else "0-1")
            else:
                f.write("1-0" if self.current_player == 'black' else "0-1")
