...
class ChineseChess:
    ...
    def load_selected_game(self, event=None):
        ...
        try:
            ...
            # Convert PGN moves to board moves
            moves = moves_text.split()
            current_board = [row[:] for row in self.board]
            
            # Process each move
            for i in range(1, len(moves)-1, 3):  # Skip move numbers and process pairs of moves
                if i+1 < len(moves):
                    # Process red move
                    red_move = moves[i]
            ...
            for i, move in enumerate(self.move_history_records, 1):
                if (i + 1) % 2 == 0:
                    ...
                    take_up_space = ' ' * len(str(int((i + 2) / 2)))
