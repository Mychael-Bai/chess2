...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.selected_instance_tag = None
    ...
    def select_piece_from_canvas(self, event, canvas):
        ...
        if closest:
            ...
            if tags:
                # The first tag is the instance-specific tag, the second is the piece type
                instance_tag = tags[0]
                piece_type = tags[1]
                if self.available_pieces['red' if piece_type.startswith('R') else 'black'][piece_type] > 0:
                    self.piece_to_place = piece_type
                    self.source_canvas = canvas
                    # Store the instance tag for removal later
                    self.selected_instance_tag = instance_tag
    ...
    def on_click(self, event):
        ...
        if self.piece_setting_mode:
            ...
            if 0 <= row < 10 and 0 <= col < 9 and self.piece_to_place:
                ...
                # Remove the specific piece instance from the source canvas
                if self.source_canvas and self.selected_instance_tag:
                    # Delete the piece instance and its highlight
                    self.source_canvas.delete(self.selected_instance_tag)
                    self.source_canvas.delete('highlight')
                
                # Clear the selection if there are no more pieces of this type
