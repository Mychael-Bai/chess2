...
class ChineseChess:
    ...
    def create_pieces_frame(self):
        ...
        def create_piece_section(frame, color):
            canvas = tk.Canvas(
                frame,
            ...
            # Draw each piece in the available pieces list
            for piece_info in self.available_pieces[color]:
                x = piece_info['col'] * self.cell_size + self.cell_size // 2
                y = piece_info['row'] * self.cell_size + self.cell_size // 2
                ...
                circle_id = canvas.create_oval(
                    ...
                    x + self.piece_radius, y + self.piece_radius,
                    fill='white',
                    outline='red' if color == 'red' else 'black',
                    width=2,
                    tags=(piece_info['instance_id'],)
                )
                
                # Draw piece text
                text_id = canvas.create_text(
                    x, y,
                    text=piece_info['piece'][1],
                    fill='red' if color == 'red' else 'black',
                    font=('KaiTi', 25, 'bold'),
                    tags=(piece_info['instance_id'],)
                )
                
                # Store the canvas IDs
                piece_info['canvas_id'] = [circle_id, text_id]
        ...
        self.red_canvas = create_piece_section(bottom_frame, 'red')
        self.black_canvas = create_piece_section(top_frame, 'black')
    ...
    def select_piece_from_canvas(self, event, canvas):
        ...
        if closest:
            ...
            if tags:
                instance_id = tags[0]
                # Find the piece info from available pieces
                color = 'red' if canvas == self.red_canvas else 'black'
                for piece_info in self.available_pieces[color]:
                    if piece_info['instance_id'] == instance_id:
                        self.piece_to_place = piece_info
                        ...
                        # Get the coordinates
                        bbox = canvas.bbox(piece_info['canvas_id'][0])  # Use the circle's bbox
    ...
    def on_click(self, event):
        ...
        if self.piece_setting_mode:
            ...
            if 0 <= row < 10 and 0 <= col < 9 and self.piece_to_place:
                # Place the piece
                self.board[row][col] = self.piece_to_place['piece']
                
                # Remove the piece from the canvas and available pieces list
                if self.source_canvas:
                    for canvas_id in self.piece_to_place['canvas_id']:
                        self.source_canvas.delete(canvas_id)
                    self.source_canvas.delete('highlight')
                    
                    color = 'red' if self.piece_to_place['piece'].startswith('R') else 'black'
                    self.available_pieces[color].remove(self.piece_to_place)
